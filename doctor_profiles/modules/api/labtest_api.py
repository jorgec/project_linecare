...
from doctor_profiles.serializers.labtest_serializers import (
    LabTestSerializer,
    PatientLabTestRequestSerializer,
    ...
    def get(self, request, *args, **kwargs):
        ...
        if s:
            labtests = LabTest.objects.annotate(
                search=SearchVector("description", "purpose", "indication", "usage")
            ).filter(Q(search=s) | Q(name__icontains=s) | Q(aliases__icontains=s))
        else:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    "That test has already been requested.",
...
class ApiPrivatePatientLabTestList(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = PatientLabTestRequestSerializer(
            checkup.get_requested_tests(), many=True
        )
...
class ApiPrivatePatientDismissedLabTestList(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = PatientLabTestRequestSerializer(
            checkup.get_dismissed_tests(), many=True
        )
...
class ApiPrivatePatientLabTestRemove(APIView):
    ...
    def post(self, request, *args, **kwargs):
        patient_labtest = get_object_or_404(
            PatientLabTestRequest,
            lab_test_id=request.GET.get("id", None),
...
class ApiPrivatePatientLabTestUndismiss(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if patient_labtest.removed_by == doctor:
            ...
            return Response(
                f"Only {patient_labtest.removed_by} can undismiss this entry",
                status=status.HTTP_403_FORBIDDEN,
            )
