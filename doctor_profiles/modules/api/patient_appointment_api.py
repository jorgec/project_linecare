    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get("medical_institution_id", None):
            medical_institution = get_object_or_404(
                MedicalInstitution.objects.get(
                    id=request.GET.get("medical_institution_id")
                )
            )
...
class ApiPatientAppointmentUpdateStatus(APIView):
    ...
    def get(self, request, *args, **kwargs):
        appointment = get_object_or_404(
            PatientAppointment, id=request.GET.get("appointment_id", None)
        )
        queue_status = request.GET.get("queue_status", None)
        if not queue_status:
            return Response(
                "Status parameter is required", status=status.HTTP_400_BAD_REQUEST
            )

        if not update_status_permissions_check(appointment, request, queue_status):
            return Response(
                f"You are not allowed to perform this action: {queue_status}",
        ...
        return Response(
            f"Status of {appointment} changed to {queue_status}",
            status=status.HTTP_200_OK,
        )


class ApiPatientAppointmentHistory(APIView):
    """
    Get patient's appointments with doctor
    patient_id=id
    [optional]
...
class ApiPatientAppointmentList(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if type(profile_type) != DoctorProfile:
            ...
            return Response(
                "This is not your schedule", status=status.HTTP_401_UNAUTHORIZED
            )

        medical_institution_id = request.GET.get("medical_institution", None)
        if medical_institution_id:
            medical_institution = get_object_or_404(
                MedicalInstitution, id=medical_institution_id
            )
