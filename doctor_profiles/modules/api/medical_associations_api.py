...
class ApiPublicMedicalAssociationDetail(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if id is not None:
            try:
                medical_association = MedicalAssociation.objects.get(id=id, is_approved=True)
            except MedicalAssociation.DoesNotExist:
                medical_association = None
        elif slug is not None:
            try:
                medical_association = MedicalAssociation.objects.get(slug=slug, is_approved=True)
            except MedicalAssociation.DoesNotExist:
                medical_association = None
        else:
            medical_association = None
...
class ApiPrivateDoctorAssociationUpdate(APIView):
    ...
    def post(self, request, *args, **kwargs):
        doctor_association = get_object_or_404(DoctorAssociation, id=request.GET.get('association', None),
        ...
        if serializer.is_valid():
            ...
            doctor_association.year_attained = serializer.validated_data['year_attained']
            doctor_association.save()
            return_serializer = DoctorAssociationSerializer(doctor_association)
            return Response(return_serializer.data, status=status.HTTP_200_OK)
        else:
